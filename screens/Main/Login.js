...
import { useContext, useState } from 'react';
import { View, Text, Button, StyleSheet, TextInput, TouchableOpacity } from 'react-native';
import { InteractiveBrowserProvider, IPublicClientApplication, PublicClientApplication } from 'react-native-msal';
...
function Login() {
  ...
  const onLoginSuccess = async () => {
    // Assuming `Instance` is your MSAL instance, replace it with the actual instance
    // You should customize this part based on the actual structure of your MSAL implementation
    let token = '';
    let userLevel = '';

    // Replace the following lines with your actual MSAL methods to get user info and token
    // For example, if Instance.getUserInfo() and Instance.getToken() are correct
    await Instance.getUserInfo().then(result => {
      // Get name, token, and email address from result
      const username = `${result.givenName} ${result.surname}`;
      const email = result.mail;

      // Check whether the user is an admin or a student with RegEx
      const condition = new RegExp('([a-zA-Z\.]+)@davistech.edu');
      userLevel = condition.test(email) ? 'admin' : 'user';
